...
# @File    : carla_bisenetv2_single_gpu_trainner.py
# @IDE: PyCharm
"""
Bisenetv2 trainner for carla dataset
"""
import os
...
from data_provider.carla import carla_tf_io

LOG = loguru.logger
CFG = parse_config_utils.carla_cfg_v2


class BiseNetV2CarlaTrainer(object):
    """
    ...
    def train(self):
        ...
        for epoch in range(epoch_start_pt, self._train_epoch_nums):
            ...
            if epoch % self._snapshot_epoch == 0:
                if self._enable_miou:
                    snapshot_model_name = 'carla_train_miou={:.4f}.ckpt'.format(train_epoch_mious)
                    ...
                    snapshot_model_name = 'carla_train_loss={:.4f}.ckpt'.format(train_epoch_losses)
                    snapshot_model_path = ops.join(self._model_save_dir, snapshot_model_name)
                    os.makedirs(self._model_save_dir, exist_ok=True)
                    self._saver.save(self._sess, snapshot_model_path, global_step=epoch)
